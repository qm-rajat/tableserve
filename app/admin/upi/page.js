...
export default function AdminUPI() {
  ...
  useEffect(() => {
    fetch('/api/upi-config').then(r => r.json()).then(data => {
      setForm({ upiId: data.upiId || '', merchantName: data.merchantName || '' })
      setLoading(false)
    })
  }, [])
    ...
    <div className="max-w-lg">
      ...
      <form onSubmit={handleSubmit} className="card space-y-5">
        <div className="flex items-center gap-3 p-4 bg-purple-50 rounded-xl">
          ...
          <div>
            ...
            <p className="text-xs text-purple-600">Customers click "Pay via UPI" and are taken directly to their UPI app. No payment gateway needed.</p>
          </div>
        </div>
        ...
        <div>
          ...
          <input
            type="text"
            className="input"
            placeholder="yourshop@upi or yourshop@okaxis"
            value={form.upiId}
            onChange={e => setForm(f => ({ ...f, upiId: e.target.value }))}
            required
          />
          <p className="text-xs text-stone-400 mt-1">This is the VPA (Virtual Payment Address) of your merchant account</p>
        </div>
        ...
        <div>
          ...
          <input
            type="text"
            className="input"
            placeholder="e.g. TableServe Cafe"
            value={form.merchantName}
            onChange={e => setForm(f => ({ ...f, merchantName: e.target.value }))}
            required
          />
          <p className="text-xs text-stone-400 mt-1">This name appears on the customer's UPI payment screen</p>
        </div>
      ...
      <div className="mt-5 card bg-amber-50 border-amber-200">
        ...
        <ul className="text-xs text-amber-700 space-y-1.5 list-disc list-inside">
          ...
          <li>No automatic payment gateway callback — verification is manual and reliable</li>
        </ul>
      </div>
    </div>
  )
