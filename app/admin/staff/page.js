...
export default function AdminStaff() {
  ...
  const openEdit = (s) => {
    setEditing(s.id)
    setForm({ name: s.name, email: s.email, password: '', pin: s.pin || '', role: s.role, phone: s.phone || '' })
    setShowForm(true)
  }
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
        {staff.map(s => (
          <div key={s.id} className={`card flex items-center gap-4 ${!s.isActive ? 'opacity-60' : ''}`}>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => toggleActive(s)} className={s.isActive ? 'text-green-500' : 'text-stone-300'}>
                {s.isActive ? <FiToggleRight className="text-2xl" /> : <FiToggleLeft className="text-2xl" />}
              </button>
