...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const todayOrders = validOrders.filter(o => new Date(o.createdAt).toDateString() === new Date().toDateString())
      const todayRevenue = todayOrders.filter(o => o.paymentStatus === 'PAID_UPI' || o.paymentStatus === 'PAID_OFFLINE')
        .reduce((s, o) => s + o.totalAmount, 0)
      setStats({ tables: validTables.length, menu: validMenu.length, staff: validStaff.length, todayOrders: todayOrders.length, todayRevenue })
    })
    .catch(() => { setStats({ tables: 0, menu: 0, staff: 0, todayOrders: 0, todayRevenue: 0 }) })
  }, [])
