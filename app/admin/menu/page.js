...
export default function AdminMenu() {
  ...
  const openEdit = (item) => {
    setEditing(item.id)
    setForm({ name: item.name, categoryId: item.categoryId, description: item.description || '', price: item.price, foodType: item.foodType, isAvailable: item.isAvailable, imageUrl: item.imageUrl || '' })
    setShowForm(true)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.ok) { toast.success(editing ? 'Item updated' : 'Item added'); closeForm(); fetch_() }
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="flex gap-2 items-center">
                  <input type="text" className="input" placeholder="Image URL or upload" value={form.imageUrl} onChange={e => setForm(f => ({ ...f, imageUrl: e.target.value }))} />
        ...
        {filtered.map(item => (
          <div key={item.id} className={`card ${!item.isAvailable ? 'opacity-60' : ''}`}>
            <div className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-1">
                  <p className="font-bold text-stone-800 text-sm leading-tight">{FOOD_BADGE[item.foodType]} {item.name}</p>
                  <button onClick={() => toggleAvailable(item)} className={`shrink-0 ${item.isAvailable ? 'text-green-500' : 'text-stone-300'}`}>
                    {item.isAvailable ? <FiToggleRight className="text-xl" /> : <FiToggleLeft className="text-xl" />}
                  </button>
                </div>
            ...
            <div className="flex gap-2 mt-3">
              <button onClick={() => openEdit(item)} className="flex-1 flex items-center justify-center gap-1.5 text-xs btn-secondary py-2">
                <FiEdit2 /> Edit
              </button>
              <button onClick={() => deleteItem(item.id)} className="flex items-center justify-center gap-1.5 text-xs bg-red-50 text-red-500 hover:bg-red-100 px-3 py-2 rounded-xl">
                <FiTrash2 />
              </button>
            </div>
          </div>
