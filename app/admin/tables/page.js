...
export default function AdminTables() {
  ...
  const openEdit = (t) => { setEditing(t.id); setForm({ number: t.number, capacity: t.capacity, locationLabel: t.locationLabel || '' }); setShowForm(true) }
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.ok) { toast.success(editing ? 'Table updated' : 'Table added'); closeForm(); fetch_() }
  ...
  const deleteTable = async (id) => {
    ...
    else toast.error('Cannot delete table with orders')
  }

  const toggleActive = async (t) => {
    await fetch(`/api/tables/${t.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isActive: !t.isActive }) })
    fetch_()
  }
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <div className="flex gap-3 mt-5">
              <button type="button" onClick={closeForm} className="flex-1 btn-secondary text-sm">Cancel</button>
          ...
          <div key={t.id} className="card">
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-stone-400">{t.locationLabel || 'No location'}</p>
              </div>
              <button onClick={() => toggleActive(t)} className={`p-1 rounded-lg ${t.isActive ? 'text-green-500' : 'text-stone-300'}`}>
                {t.isActive ? <FiToggleRight className="text-2xl" /> : <FiToggleLeft className="text-2xl" />}
              </button>
            </div>
            <p className="text-sm text-stone-600 mb-4">👥 {t.capacity} seats · <span className={t.isActive ? 'text-green-600 font-semibold' : 'text-stone-400'}>{t.isActive ? 'Active' : 'Inactive'}</span></p>
            <div className="flex gap-2">
              <button onClick={() => openEdit(t)} className="flex-1 flex items-center justify-center gap-1.5 text-xs btn-secondary py-2">
                <FiEdit2 /> Edit
              </button>
              <button onClick={() => deleteTable(t.id)} className="flex items-center justify-center gap-1.5 text-xs bg-red-50 text-red-500 hover:bg-red-100 px-3 py-2 rounded-xl">
                <FiTrash2 />
              </button>
            </div>
          </div>
