...
function ConfirmPage() {
  ...
  if (!order)  return <div className="min-h-screen flex items-center justify-center"><p className="text-stone-500">Order not found</p></div>
  ...
  const steps = [
    ...
    { label: 'Being Prepared', done: isPaid,            icon: <FiClock /> },
    { label: 'Delivered',      done: order.is_delivered, icon: <FiPackage /> },
  ]
      ...
      <div className="max-w-md mx-auto px-4 py-8">
        ...
        <div className="card mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-stone-400">{order.table?.location_label}</p>
          </div>
          ...
          <div className="space-y-3">
            {steps.map((step, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step.done ? 'bg-green-500 text-white' : 'bg-stone-100 text-stone-400'}`}>
                  {step.icon}
                </div>
                <span className={`font-medium text-sm ${step.done ? 'text-stone-800' : 'text-stone-400'}`}>{step.label}</span>
                {i === 1 && !step.done && <span className="ml-auto text-xs text-stone-400 animate-pulse">Waiting...</span>}
          ...
          <div className="space-y-2">
            {order.order_items?.map(item => (
              <div key={item.id} className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-stone-800">₹{item.unit_price * item.quantity}</span>
              </div>
            ))}
