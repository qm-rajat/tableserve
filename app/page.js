      ...
      <div className="max-w-md mx-auto px-4 py-6">
        ...
        {loading ? (
          <div className="grid grid-cols-2 gap-3">
            {[...Array(6)].map((_, i) => <div key={i} className="h-32 bg-stone-100 rounded-2xl animate-pulse" />)}
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3">
            {tables.map(table => {
              ...
              return (
                <button key={table.id} onClick={() => handleTableSelect(table)} disabled={!isClickable}
                  ...
                  <div className={`absolute top-3 right-3 w-2.5 h-2.5 rounded-full ${
                    status === 'available' ? 'bg-green-400' : status === 'occupied' ? 'bg-amber-400' : 'bg-stone-300'}`} />
                  ...
                  <div className="flex items-center gap-1 text-xs text-stone-500 mb-1">
                    <FiUsers className="text-xs" /><span>{table.capacity} seats</span>
                  </div>
                  {table.location_label && (
                    <div className="flex items-center gap-1 text-xs text-stone-400">
                      <FiMapPin className="text-xs" /><span>{table.location_label}</span>
                    </div>
                  )}
                  {status === 'occupied' && (
